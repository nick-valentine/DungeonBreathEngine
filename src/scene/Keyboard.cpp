...
namespace scene {
    Keyboard::Keyboard(sf::Vector2i size) :
        Scene(size),
    ...
    void Keyboard::update(int delta, sf::RenderWindow &window)
    {
    ...
    Scene::Status Keyboard::status()
    {
        return this->state;
    }

    Scene *Keyboard::new_scene()
    {
        return this->next_scene;
    }

    void Keyboard::set_input(sf::String input)
    {
        this->text_input = input;
        this->input_label.set_string(this->text_input);
    }

    sf::String Keyboard::get_input()
    {
        return this->text_input;
    }

    void Keyboard::add_button_range(char start, int count, int start_x, int start_y)
    {
        const int step = size.x / 10;
