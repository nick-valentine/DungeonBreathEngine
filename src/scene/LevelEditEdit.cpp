#include "LevelEditEdit.h"

#define TILE_SIZE 64
#define CAMERA_LAG 10

namespace scene {
    Editor::Cursor::Cursor() : size(TILE_SIZE, TILE_SIZE)
    {
        shape.setSize(sf::Vector2f(TILE_SIZE, TILE_SIZE));
        shape.setOutlineColor(sf::Color::Blue);
        shape.setOutlineThickness(5.0f);
        shape.setFillColor(sf::Color::Transparent);
        shape.setPosition(sf::Vector2f(0.0f, 0.0f));
    }

    void Editor::Cursor::move(sf::Vector2i offset)
    {
        this->loc += offset;
        shape.move(offset.x, offset.y);
        if (current != nullptr) {
            current->set_location(this->loc);
        }
    }

    sf::Vector2i Editor::Cursor::get_location()
    {
        return loc;
    }

    void Editor::Cursor::draw(sf::RenderWindow &window)
    {
        window.draw(shape);
        if (current != nullptr) {
            current->draw(window);
        }
    }

    void Editor::Cursor::set_tile(render::Tile *tile)
    {
        current.reset(tile);
        size = current->get_size();
        current->set_location(loc);
        shape.setSize(sf::Vector2f(size.x, size.y));
        this->actor = "";
    }

    void Editor::Cursor::set_actor(std::string actor)
    {
        this->actor = actor;
        current.reset(nullptr);
    }

    std::string Editor::Cursor::get_actor()
    {
        return this->actor;
    }

    LevelEditEdit::LevelEditEdit(sf::Vector2i size, std::string name) : 
        Scene(size), name(name), actor_selector(size),
        edit(sf::IntRect(size.x - 400, size.y - 220, 300, 50), core::StringProvider::get("leveleditmenu.edit")),
        save(sf::IntRect(size.x - 400, size.y - 160, 300, 50), core::StringProvider::get("leveleditmenu.save")),
        back(sf::IntRect(size.x - 400, size.y - 100, 300, 50), core::StringProvider::get("leveleditmenu.back_button")),
        last_input(core::Input::num_keys, false)
    {
        load_level();
        this->main_window.reset(sf::FloatRect(0, 0, size.x, size.y));
        this->menu_window.reset(sf::FloatRect(0, 0, size.x, size.y));

        layer_input.add("layer", menu);
        collision_type.add("collision", menu);
        menu.add_button("edit", &edit);
        menu.add_button("save", &save);
        menu.add_button("exit_menu", &back);
    }

    void LevelEditEdit::update(int delta, sf::RenderWindow &window)
    {
        switch (cur_state) {
        case edit_level:
            return update_edit(delta, window);
        case select_tile:
            return update_select(delta, window);
        case select_actor:
            return update_actor_select(delta, window);
        case in_menu:
            return update_menu(delta, window);
        }
    }

    void LevelEditEdit::draw(sf::RenderWindow &window)
    {
        switch (cur_state) {
        case edit_level:
            return draw_edit(window);
        case select_tile:
            return draw_select(window);
        case select_actor:
            return draw_actor_select(window);
        case in_menu:
            return draw_menu(window);
        }
    }

    Scene::Status LevelEditEdit::status()
    {
        return this->state;
    }

    Scene *LevelEditEdit::new_scene()
    {
        return this->next_scene;
    }

    void LevelEditEdit::reset_status()
    {
        this->next_scene = nullptr;
        this->state = Scene::Status::nothing;
    }

    void LevelEditEdit::load_level()
    {
        std::string filename = name + ".txt";
        std::ifstream ifile(LEVELDIR + filename);
        core::app_container.get_logger()->info(filename.c_str());
        if (!ifile.good()) {
            throw core::FileNotFoundException();
        }
        std::getline(ifile, title);
        ifile>>tileset;
        ifile.close();
        tiles = std::unique_ptr<render::TileSet>(new render::TileSet(tileset));
        world = std::unique_ptr<play::World>(
            new play::World(
                tileset, 
                std::unique_ptr<play::WorldGenerator>(
                    new play::WorldLoader(LEVELDIR + filename)
                )
            )
        );
        world->set_edit_mode(true);
        tile_selector = std::unique_ptr<LevelEditTile>(
            new LevelEditTile(
                this->size,
                tileset
            )
        );
    }

    void LevelEditEdit::update_edit(int delta, sf::RenderWindow &window)
    {
        world->update(delta, window);
        auto new_input = core::app_container.get_input()->poll_all();
        if (new_input[core::Input::accept] && (new_input[core::Input::fire] && !last_input[core::Input::fire])) {
            world->remove_tile(layer, cursor.get_location() / TILE_SIZE);
        } else if (new_input[core::Input::accept] && (new_input[core::Input::alt_fire] && !last_input[core::Input::alt_fire])) {
            this->cur_state = select_actor;
        } else if (new_input[core::Input::down] && !last_input[core::Input::down]) {
            cursor.move(sf::Vector2i(0, TILE_SIZE));
        } else if (new_input[core::Input::up] && !last_input[core::Input::up]) {
            cursor.move(sf::Vector2i(0, -TILE_SIZE));
        } else if (new_input[core::Input::left] && !last_input[core::Input::left]) {
            cursor.move(sf::Vector2i(-TILE_SIZE, 0));
        } else if (new_input[core::Input::right] && !last_input[core::Input::right]) {
            cursor.move(sf::Vector2i(TILE_SIZE, 0));
        } else if (!new_input[core::Input::escape] && last_input[core::Input::escape]) {
            this->cur_state = in_menu;
        } else if (new_input[core::Input::alt_fire] && !last_input[core::Input::alt_fire]) {
            this->cur_state = select_tile;
        } else if (new_input[core::Input::fire] && !last_input[core::Input::fire]) {
            if (selected_tile != -1) {
                world->set_tile(tiles->spawn(selected_tile, cursor.get_location()), layer, cursor.get_location() / TILE_SIZE);
            } else {
                world->add_actor(cursor.get_actor(), cursor.get_location());
            }
        } else if (new_input[core::Input::next] && !last_input[core::Input::next]) {
            world->add_collision(collision, cursor.get_location());
        }
        last_input = new_input;
        auto target_camera_center = cursor.get_location();
        auto camera_center = this->main_window.getCenter();
        sf::Vector2f camera_diff;
        camera_diff.x = (target_camera_center.x - camera_center.x) / CAMERA_LAG;
        camera_diff.y = (target_camera_center.y - camera_center.y) / CAMERA_LAG;

        this->main_window.move(camera_diff);
    }

    void LevelEditEdit::draw_edit(sf::RenderWindow &window)
    {
        window.setView(this->main_window);
        world->draw(window);
        cursor.draw(window);
    }

    void LevelEditEdit::update_select(int delta, sf::RenderWindow &window)
    {
        this->tile_selector->update(delta, window);
        if (this->tile_selector->status() == Scene::Status::pop_scene) {
            this->tile_selector->reset_status();
            this->selected_tile = this->tile_selector->get_selected();
            if (this->selected_tile != -1) {
                this->cursor.set_tile(tiles->spawn(selected_tile, cursor.get_location()));
            }
            this->cur_state = edit_level;
        }
    }

    void LevelEditEdit::draw_select(sf::RenderWindow &window)
    {
        this->tile_selector->draw(window);
    }

    void LevelEditEdit::update_actor_select(int delta, sf::RenderWindow &window)
    {
        this->actor_selector.update(delta, window);
        if (this->actor_selector.status() == Scene::Status::pop_scene) {
            this->actor_selector.reset_status();
            this->cursor.set_actor(this->actor_selector.get_selected());
            this->cur_state = edit_level;
            this->selected_tile = -1;
        }
    }

    void LevelEditEdit::draw_actor_select(sf::RenderWindow &window)
    {
        this->actor_selector.draw(window);
    }

    void LevelEditEdit::update_menu(int delta, sf::RenderWindow &window)
    {
        menu.update(delta, core::app_container.get_input(), window);

        std::string pressed = this->menu.neg_edge_button();

        layer_input.update(pressed);
        collision_type.update(pressed);
        if (pressed == "layer_left") {
            this->layer = this->layer_input.value();
        } else if (pressed == "layer_right") {
            this->layer = this->layer_input.value();
        } else if (pressed == "collision_left") {
            this->collision = this->collision_type.value();
        } else if (pressed == "collision_right") {
            this->collision = this->collision_type.value();
        } else if (pressed == "edit") {
            this->cur_state= edit_level;
        } else if (pressed == "exit_menu") {
            this->next_scene = nullptr;
            this->state = Scene::pop_scene;
        } else if (pressed == "save") {
            this->world->save();
        }
    }

    void LevelEditEdit::draw_menu(sf::RenderWindow &window)
    {
        window.setView(this->menu_window);
        layer_input.draw(window);
        collision_type.draw(window);
        menu.draw(window);
    }

    void LevelEditEdit::update_layer(int diff)
    {
        layer+=diff;
        if (layer < 0) {
            layer = 0;
        }
    }
};
